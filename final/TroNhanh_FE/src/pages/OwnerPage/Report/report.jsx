...
const Report = () => {
  ...
  const fetchBookedAccommodations = async () => {
    try {
      ...
      const accRes = await axios.get(`https://tronhanh-be.onrender.com/api/accommodation?ownerId=${user._id}`);
      ...
      const accommodationsWithCustomer = await Promise.all(
        accommodationsToProcess.map(async (acc) => {
          try {
            ...
            const bookingRes = await axios.get(`https://tronhanh-be.onrender.com/api/booking/accommodation/${acc._id}`);
