...
export default function FeedbackScreen() {
    ...
    // Nếu không có bookingId, có thể sử dụng default để test
    useEffect(() => {
        const checkAuth = async () => {
            ...
            if (!bookingId) {
                console.log("No bookingId found, using test mode");
            } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (bookingId && token) {
                ...
                console.log("Final feedback text:", feedbackWithTags);
                await sendFeedback(bookingId, feedbackWithTags, rating);
                console.log("Feedback sent successfully");
            } else {
                if (!token) {
                    console.log("No auth token - simulating feedback submission");
                }
                if (!bookingId) {
                    console.log("No bookingId - simulating feedback submission");
                }
                console.log("Simulated feedback:", {
                    bookingId: bookingId || "test-booking-id",
                    rating,
                    tags: selectedTags,
                    text: feedbackText
                });
            ...
            // Vẫn hiển thị trang cảm ơn để demo
            setShowThankYouPage(true);
        } finally {
            setLoading(false);
        }
