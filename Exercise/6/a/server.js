...
app.post('/articles', async (req, res) => {
  try {
    res.status(200).end('Will add the article: ' + req.body.title + ' with details: ' + req.body.text);
  } catch (err) {
    res.status(400).send('Error: ' + err.message);
  }
});
...
app.post('/articles/:id', async (req, res) => {
  try {
    res.status(200).end('POST operation not supported on /articles/' + req.params.id);
  } catch (err) {
    res.status(400).send('Error: ' + err.message);
  }
});
...
app.put('/articles/:id', async (req, res) => {
  try {
    ...
    res.status(200).end('Will update the article: ' + req.params.id + ' with details: ' + req.body.text);
  } catch (err) {
    res.status(400).send('Error: ' + err.message);
  }
});
